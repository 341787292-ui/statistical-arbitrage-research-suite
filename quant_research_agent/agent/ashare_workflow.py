...
def run_ashare_diagnostic_agent(
    *,
    ...
    record(
        ...
        "Created sign, residual-mechanism, and turnover hypotheses.",
    )
    ...
    if mechanism["paper_direction_sharpe_above_half"]:
        verdict = (
            "Residual mean reversion is present in the paper direction, while stock-space "
            "mapping and turnover remain the primary implementation problems."
        )
        next_action = (
            "Keep OU parameters frozen and test slower signal-to-portfolio mapping with "
            "explicit turnover budgets on the development sample."
        )
        h2_status = "supported"
        ...
        next_action = (
            "Audit residual construction, PCA refit continuity, and one-day timing before "
            "running any parameter or model search."
        )
        h2_status = "rejected"
    else:
    ...
    report = _render_report(contract, evidence, hypotheses, experiment, final, trace)
